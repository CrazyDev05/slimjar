...
import me.lucko.jarrelocator.transformers.ServicesResourceTransformer;
import me.lucko.jarrelocator.transformers.XMLResourceTransformer;
...
public final class JarRelocator {
    ...
    public void run() throws IOException {
        ...
        List<ResourceTransformer> transformers = List.of(
                new ServicesResourceTransformer(),
                new XMLResourceTransformer()
        );
        ...
        try (JarOutputStream out = new JarOutputStream(new BufferedOutputStream(new FileOutputStream(this.output)))) {
            try (JarFile in = new JarFile(this.input)) {
                JarRelocatorTask task = new JarRelocatorTask(this.remapper, out, in, transformers);
                task.processEntries();
            }
        }
    }
